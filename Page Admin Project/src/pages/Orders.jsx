...
const getStatusCfg = (status, t) => {
  ...
  const configs = {
    ...
    DELIVERED: { label: t("status_delivered", "Đã giao"), color: "bg-emerald-500/10 text-emerald-600 border-emerald-200/50", icon: "pi-check-circle" },
    CANCELLED: { label: t("status_cancelled", "Đã hủy"), color: "bg-red-500/10 text-red-600 border-red-200/50", icon: "pi-times-circle" }
  };
...
export default function Orders() {
  ...
  const getFilteredOrders = () => {
    ...
    // PENDING + PROCESSING: cần Admin3 xử lý (PENDING = chờ Admin1 duyệt, PROCESSING = Admin1 đã duyệt, chờ giao)
    if (activeTab === "PENDING") return orders.filter(o => 
      o.Status.includes("PENDING") || o.Status.includes("AWAITING") || 
  ...
  const statusBodyTemplate = (rowData) => {
    const cfg = getStatusCfg(rowData.Status, t);
    return (
  ...
  const amountBodyTemplate = (rowData) => {
    return (
      <span className="font-black text-slate-900 tracking-tighter">
        {new Intl.NumberFormat("en-US").format(Number(rowData.TotalAmount)) + "$"}
      </span>
    );
  };
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 font-inter">
      ...
      {isOpsAdmin && currentUser?.canAccessCustomerInfo === false ? (
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center bg-white rounded-3xl p-10 shadow-sm border border-red-100 mt-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div>
          <span className="px-3 py-1 bg-cyan-100 text-cyan-600 text-[10px] font-black uppercase tracking-widest rounded-lg mb-3 inline-block">{t("management", "Management")}</span>
          <h1 className="text-4xl font-black tracking-tighter text-slate-900">
            {t("orders_list", "Danh sách Đơn hàng")}
          </h1>
          <p className="text-slate-500 mt-2 font-medium">{t("orders_list_desc", "Hệ thống xử lý và vận hành đơn hàng chuyên nghiệp.")}</p>
        </div>
        <div className="flex items-center gap-3">
           <div className="relative">
             ...
             <InputText
               ...
               placeholder={t("search_order_placeholder", "Tìm mã ĐH, khách hàng...")}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center justify-between">
             <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{t("total_orders_short", "Tổng đơn")}</p>
           ...
           <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center justify-between">
              <div>
                 <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-1">{t("need_processing", "Cần xử lý")}</p>
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-center justify-between">
             <div>
                <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest mb-1">{t("shipping_short", "Đang giao")}</p>
                <h3 className="text-3xl font-black text-slate-800">
      ...
      <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2 scrollbar-hide">
         {[
           { id: "ALL", label: t("filter_all", "Tất cả"), icon: "pi-list" },
           { id: "PENDING", label: t("status_pending", "Chờ xử lý"), icon: "pi-clock" },
           { id: "SHIPPING", label: t("status_shipping", "Đang giao"), icon: "pi-truck" },
           { id: "COMPLETED", label: t("status_completed", "Đã hoàn thành"), icon: "pi-check-circle" }
         ].map(tab => (
           <button
             key={tab.id}
      ...
      <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-50 overflow-hidden">
        <DataTable
          ...
          emptyMessage={<div className="py-10 text-center text-slate-400 font-bold">{t("no_orders_found", "Không tìm thấy đơn hàng nào")}</div>}
          className="p-datatable-modern"
          rowHover
          responsiveLayout="stack"
          breakpoint="960px"
        >
          <Column field="OrderID" header={t("order_id_col", "MÃ ĐH")} sortable style={{ width: '12%' }}
            body={(r) => <span className="font-black text-cyan-600">#{r.OrderID}</span>} />
          <Column header={t("customer_col", "KHÁCH HÀNG")} body={customerBodyTemplate} style={{ width: '28%' }} />
          <Column field="OrderDate" header={t("order_date_col", "NGÀY ĐẶT")} sortable 
             body={(r) => <span className="text-xs font-bold text-slate-500">{r.OrderDate ? new Date(r.OrderDate).toLocaleDateString("vi-VN") : "—"}</span>} />
          <Column field="TotalAmount" header={t("total_amount_col", "TỔNG TIỀN")} sortable body={amountBodyTemplate} />
          <Column field="PaymentMethod" header={t("payment_method_col", "P.THỨC")} sortable 
          ...
          <Column header={t("status_col", "TRẠNG THÁI")} body={statusBodyTemplate} sortable field="Status" style={{ width: '18%' }} />
      ...
      <Dialog 
        ...
        header={t("order_details", "Chi tiết Đơn hàng")} 
        modal 
          ...
          <div className="flex flex-col bg-slate-50/50 min-h-[500px]">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-6">
                <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
                   <h3 className="text-[10px] font-black text-cyan-600 uppercase tracking-[2px] mb-4">{t("customer_info", "Thông tin khách")}</h3>
                   <div className="space-y-3">
                      <div className="flex items-center gap-3">
                         ...
                         <div className="flex flex-col">
                            <span className="text-xs font-black text-slate-700 uppercase">{selectedOrder.FullName || t("guest", "Khách vãng lai")}</span>
                            <span className="text-[10px] font-bold text-slate-400">{t("customer_name_label", "Tên khách hàng")}</span>
                         </div>
                      </div>
                      <div className="flex items-center gap-3">
                         ...
                         <div className="flex flex-col">
                            ...
                            <span className="text-[10px] font-bold text-slate-400">{t("phone_label", "Số điện thoại")}</span>
                         </div>
                      </div>
                      <div className="flex items-center gap-3">
                         ...
                         <div className="flex flex-col min-w-0">
                            ...
                            <span className="text-[10px] font-bold text-slate-400">{t("email_label", "Email")}</span>
                         </div>
                      </div>
                   </div>
                </div>
                ...
                <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 flex flex-col">
                   <h3 className="text-[10px] font-black text-indigo-600 uppercase tracking-[2px] mb-4">{t("shipping_address", "Địa chỉ giao hàng")}</h3>
                   ...
                   <div className="mt-4 pt-4 border-t border-slate-50 flex justify-between items-center">
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t("payment_method_short", "Phương thức")}</span>
             ...
             <div className="px-6 pb-6">
                <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                   <div className="bg-slate-50 px-6 py-3 border-b border-slate-50">
                      <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[2px]">{t("ordered_products", "Sản phẩm đã đặt")}</h3>
                   </div>
                   <div className="max-h-[300px] overflow-y-auto p-4 space-y-4">
                      ...
                      <div className="flex flex-col items-center justify-center py-10 opacity-30">
                         ...
                         <p className="text-xs font-black tracking-widest uppercase">{t("expand_items_desc", "Mở rộng xem chi tiết Items...")}</p>
                      </div>
                   </div>
                </div>
             </div>
             ...
             <div className="mt-auto bg-white p-6 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="flex items-center gap-3">
                   <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t("status_label_colon", "Trạng thái:")}</span>
                   {statusBodyTemplate(selectedOrder)}
                </div>
                <div className="flex gap-3">
                   {isSuperAdmin && (selectedOrder.Status.includes("PENDING") || selectedOrder.Status === "Chờ xử lý") && (
                      <Button 
                        label={t("approve_order_btn", "Duyệt Đơn")} 
                        severity="success" 
                        size="small" 
                   ...
                   {(isSuperAdmin || isOpsAdmin) && !selectedOrder.Status.includes("CANCEL") && (
                      <Button 
                        label={t("update_shipping_btn", "Cập nhật vận chuyển")} 
                        severity="info" 
                        size="small" 
                        outlined 
      ...
      {/* Approve Dialog */}
      <Dialog visible={approveDialog} style={{ width: "450px" }} header={t("approve_order_header", "Phê duyệt Đơn hàng")} modal onHide={() => setApproveDialog(false)} className="shadow-2xl rounded-3xl overflow-hidden">
        <div className="flex flex-col gap-6 mt-4 p-2">
          <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
             <p className="text-emerald-700 text-sm font-bold">{t("approving_order_msg", "Bạn đang phê duyệt đơn hàng")} <b className="text-lg">#{selectedOrder?.OrderID}</b></p>
             <p className="text-xs text-emerald-600/70 mt-1">{t("notify_email_msg", "Hành động này sẽ thông báo cho khách qua Email.")}</p>
          </div>
          <div className="flex flex-col gap-2">
            <label htmlFor="reason" className="font-black text-slate-500 text-[10px] uppercase tracking-widest">{t("reject_reason_label", "Lý do từ chối (Chỉ điền nếu Hủy đơn)")}</label>
            <InputTextarea
               id="reason"
               value={rejectReason}
               ...
               placeholder={t("reject_reason_placeholder", "Khách sẽ nhận email giải thích này...")}
            />
          </div>
          <div className="flex justify-end gap-3 mt-4">
            <Button label={t("cancel_order_btn", "Hủy Bỏ Đơn")} icon="pi pi-times" outlined severity="danger" onClick={() => handleApproveReject("REJECT")} className="rounded-2xl font-black px-6" />
            <Button label={t("approve_now_btn", "Duyệt Ngay")} icon="pi pi-check" severity="success" onClick={() => handleApproveReject("APPROVE")} className="rounded-2xl font-black bg-emerald-500 hover:bg-emerald-600 border-none px-8 shadow-lg shadow-emerald-500/30" />
          </div>
        </div>
      </Dialog>

      {/* Status Dialog */}
      <Dialog visible={statusDialog} style={{ width: "400px" }} header={t("update_shipping_header", "Cập nhật Vận chuyển")} modal onHide={() => setStatusDialog(false)} className="shadow-2xl rounded-3xl overflow-hidden">
        {/* ... (phần code cũ giữ nguyên) */}
        <div className="flex flex-col gap-6 mt-4 p-2">
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
             <p className="text-xs font-bold text-slate-500">{t("order_label_colon", "Đơn hàng: ")}<b className="text-slate-900">#{selectedOrder?.OrderID}</b></p>
             <p className="text-[10px] font-black text-slate-400 uppercase mt-1">{t("status_label_colon_short", "Trạng thái: ")}{getStatusCfg(selectedOrder?.Status, t).label}</p>
          </div>
          <div className="flex flex-col gap-2">
             <label htmlFor="status" className="font-black text-slate-500 text-[10px] uppercase tracking-widest">{t("select_new_status", "Chọn trạng thái mới")}</label>
             <Dropdown
                id="status"
                value={newStatus}
                ...
                placeholder={t("new_status_placeholder", "Ví dụ: Đang giao, Đã hoàn thành...")}
             />
          </div>
          <div className="flex justify-end mt-4">
            <Button label={t("save_update_btn", "Lưu & Cập nhật")} icon="pi pi-save" onClick={handleUpdateStatus} className="rounded-2xl font-black bg-cyan-600 hover:bg-cyan-700 border-none px-10 shadow-lg shadow-cyan-500/30" />
          </div>
        </div>
      </Dialog>

      {/* Delete Confirmation Dialog */}
      <Dialog visible={deleteDialog} style={{ width: "400px" }} header={t("confirm_delete_header2", "Xác nhận xóa")} modal onHide={() => setDeleteDialog(false)} className="rounded-3xl">
        <div className="flex flex-col gap-4 p-2">
          <div className="flex items-center gap-4 text-red-500">
             <i className="pi pi-exclamation-triangle text-4xl"></i>
             <p className="font-bold">{t("confirm_delete_order", "Bạn có chắc chắn muốn xóa đơn hàng")} <b>#{selectedOrder?.OrderID}</b>?</p>
          </div>
          <p className="text-sm text-slate-500">{t("delete_order_warning", "Hành động này không thể hoàn tác và sẽ xóa vĩnh viễn dữ liệu đơn hàng.")}</p>
          <div className="flex justify-end gap-3 mt-4">
            <Button label={t("cancel", "Hủy")} text severity="secondary" onClick={() => setDeleteDialog(false)} className="font-bold" />
            <Button label={t("delete_permanently_btn", "Xóa Vĩnh Viễn")} severity="danger" onClick={handleDeleteOrder} className="rounded-2xl font-bold bg-red-500 border-none px-6" />
          </div>
        </div>
      </Dialog>

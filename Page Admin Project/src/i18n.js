import i18n from "i18next";
import { initReactI18next } from "react-i18next";

const resources = {
  vi: {
    translation: {
      // --- Topbar & Sidebar ---
      search: "Tìm kiếm...",
      search_placeholder: "Tìm kiếm sản phẩm...",
      home: "Trang chủ",
      dashboard: "Bảng điều khiển",
      users: "Người dùng",
      roles: "Vai trò",
      product: "Sản phẩm",
      products_title: "Sản phẩm",
      promotions: "Khuyến Mãi",
      login: "Đăng nhập",

      // --- Dashboard ---
      acc_revenue: "Doanh thu tích lũy",
      desc_revenue: "Tổng doanh thu bán hàng",
      acc_users: "Khách hàng đăng ký",
      desc_users: "Thành viên trên hệ thống",
      acc_orders: "Tổng đơn đặt hàng",
      desc_orders: "Đơn mua thành công",
      acc_reviews: "Lượt đánh giá",
      desc_reviews: "Phản hồi người dùng",
      welcome_back: "Chào mừng trở lại! 👋",
      dashboard_desc: "Bảng điều khiển thống kê tổng quan của hệ thống.",
      trending_categories: "Thể loại thịnh hành",
      total_products: "TỔNG SP",
      no_product_data: "Chưa có dữ liệu sản phẩm",
      revenue_7_days: "Doanh thu 7 ngày qua",
      revenue_stats_desc: "Thống kê doanh thu theo ngày",
      no_recent_transactions: "Chưa có giao dịch nào gần đây",

      // --- Notifications ---
      notifications: "Thông báo",
      notification_empty: "Không có thông báo nào",
      notif_new_order: "Đơn hàng mới",
      notif_new_review: "Đánh giá mới",
      notif_admin_action: "Hoạt động Admin",
      mark_all_read: "Đánh dấu đã đọc",
      view_all: "Xem tất cả",

      // --- Profile Popover ---
      profile: "Hồ sơ",
      settings: "Cài đặt",
      logout: "Đăng xuất",

      // --- Product Page ---
      new_product: "Thêm sản phẩm",
      add_product_title: "Thêm sản phẩm mới",
      edit_product_title: "Cập nhật sản phẩm",
      product_name: "Tên sản phẩm",
      product_price: "Giá tiền",
      select_category: "Chọn danh mục",
      save: "Lưu lại",
      delete: "Xóa sản phẩm",
      cancel: "Hủy bỏ",
      size_stock_management: "Quản lý Tồn kho Size",
      size_name: "Size (XS, M...)",
      size_stock: "Số lượng",
      add_size: "+ Thêm Size",
      no_sizes: "Trống. Tồn kho chung = 0 nếu không có size.",

      // --- Promotions Page ---
      promotions_title: "Khuyến Mãi",
      promotions_sub: "Quản lý mã giảm giá và chiến dịch",
      add_promotion: "Thêm khuyến mãi",
      promo_code: "Mã giảm giá",
      promo_discount: "Giá trị giảm",
      promo_type: "Loại",
      promo_start: "Ngày bắt đầu",
      promo_end: "Ngày kết thúc",
      promo_percent: "Phần trăm (%)",
      promo_fixed: "Cố định ($)",
      
      promotions_title_big: "Quản Lý Khuyến Mãi",
      promotions_desc_big: "Tạo và thiết lập chiến dịch Voucher siêu hấp dẫn",
      create_campaign: "Tạo Chiến Dịch",
      empty_promotions: "Chưa có chương trình khuyến mãi nào được tạo.",
      ticket_code: "MÃ TICKET",
      discount_percent: "MỨC GIẢM",
      start_date: "NGÀY BẮT ĐẦU",
      end_date: "NGÀY KẾT THÚC",
      status: "TRẠNG THÁI",
      actions: "THAO TÁC",

      // --- Roles Page ---
      roles_title: "Phân Quyền Quản Trị",
      roles_sub: "Quản lý 3 cấp độ Admin và lịch sử hoạt động",
      activity_log: "Lịch Sử Hoạt Động",
      order_flow: "Luồng Đơn Hàng",
      clear_all_logs: "Xóa tất cả",

      // --- Roles Constants ---
      role_admin1_title: "Admin Chính",
      role_admin1_sub: "Toàn quyền kiểm soát hệ thống",
      role_admin1_p1: "Xem & quản lý toàn bộ Dashboard",
      role_admin1_p2: "Quản lý tất cả người dùng (CRUD)",
      role_admin1_p3: "Quản lý & duyệt sản phẩm",
      role_admin1_p4: "Toàn quyền quản lý chiến dịch Khuyến Mãi",
      role_admin1_p5: "Duyệt đơn hàng do Admin 3 lên",
      role_admin1_p6: "Phân quyền & quản lý Admin 2, Admin 3",
      role_admin1_p7: "Xem báo cáo thống kê doanh thu",
      role_admin1_p8: "Cấu hình hệ thống",
      
      role_admin2_title: "Admin Kho",
      role_admin2_sub: "Quản lý sản phẩm & kho hàng",
      role_admin2_p1: "Thêm sản phẩm mới vào kho",
      role_admin2_p2: "Sửa thông tin, giá, ảnh sản phẩm",
      role_admin2_p3: "Tạo chiến dịch mã Code & % Giảm",
      role_admin2_p4: "Xóa sản phẩm (khi Admin 1 ra lệnh)",
      role_admin2_p5: "Xem danh sách sản phẩm & tồn kho",
      role_admin2_p6: "Xem thống kê kho hàng",
      role_admin2_b1: "Quản lý người dùng",
      role_admin2_b2: "Duyệt đơn hàng",
      role_admin2_b3: "Phân quyền Admin",
      role_admin2_b4: "Cấu hình hệ thống",
      
      role_admin3_title: "Admin Vận Hành",
      role_admin3_sub: "Xử lý đơn hàng & chăm sóc khách hàng",
      role_admin3_p1: "Nhận thông tin đặt hàng từ khách hàng",
      role_admin3_p2: "Lên đơn hàng khi Admin 1 duyệt",
      role_admin3_p3: "Xem thông tin khách hàng của đơn hàng",
      role_admin3_p4: "Cập nhật trạng thái vận chuyển",
      role_admin3_p5: "Xem lịch sử & danh sách đơn hàng",
      role_admin3_b1: "Quản lý sản phẩm",
      role_admin3_b2: "Quản lý Khuyến mãi",
      role_admin3_b3: "Phân quyền Admin",
      role_admin3_b4: "Xóa người dùng",
      role_admin3_b5: "Cấu hình hệ thống",
      
      flow_step1_actor: "Khách hàng",
      flow_step1_action: "Đặt hàng trên Web",
      flow_step2_actor: "Admin Vận Hành",
      flow_step2_action: "Lên đơn hàng",
      flow_step3_actor: "Admin Chính",
      flow_step3_action: "Duyệt / Từ chối đơn",
      flow_step4_actor: "Admin Vận Hành",
      flow_step4_action: "Giao hàng",
      
      role_admin1_exclusive: "Admin 1 có thể thực hiện MỌI quyền của Admin 2 và Admin 3",
      reset_pw: "Reset MK",
      lock_acc: "Khóa TK",
      unlock_acc: "Mở Khóa",

      // --- User Page ---
      user_management_sub: "Quản lý tài khoản và phân quyền hệ thống",
      user_table_name: "Người dùng",
      search_user_placeholder: "Tìm Tên, Số Điện Thoại...",
      users: "Người Dùng",
      
      // User Status & Detailed Strings
      status_pending_admin: "Chờ duyệt",
      status_processing: "Đang xử lý",
      status_shipping: "Đang giao",
      status_delivered: "Đã giao",
      status_cancelled: "Đã hủy",
      status_awaiting_payment: "Chờ thanh toán",
      status_approved: "Đã duyệt",
      delete_account_confirm: "Xác nhận xóa tài khoản",
      delete_user_question: "Xóa tài khoản",
      delete_warning: "Hành động này không thể hoàn tác.",
      accept_label: "Xóa",
      reject_label: "Hủy",
      filter_all: "Tất cả",
      filter_admin: "Admin",
      filter_customer: "Khách hàng",
      total_users: "Tổng người dùng",
      admins_count: "Quản trị viên",
      customers_count: "Khách hàng",
      active_users_count: "Đang hoạt động",
      no_users_found: "Không tìm thấy người dùng nào",
      role_admin_badge: "Admin",
      role_customer_badge: "Khách hàng",
      created_at: "Tạo",
      showing_users: "Hiển thị",
      users_count_label: "người dùng",
      personal_info: "Thông tin cá nhân",
      phone_label: "Điện thoại",
      not_provided: "Chưa cung cấp",
      address_label: "Địa chỉ",
      register_date_label: "Ngày đăng ký",
      status_label: "Trạng thái",
      active_status: "Đang hoạt động",
      locked_status: "Đã khóa",
      order_history: "Lịch sử đơn hàng",
      no_orders: "Chưa có đơn hàng nào",
      order_hash: "Đơn #",
      products_count_label: "sản phẩm",
      delete_this_account: "Xóa tài khoản này",

      // --- Coupons Page ---
      status_running: "Đang chạy",
      status_stopped: "Đã Dừng/Hết Hạn",
      discount_amount_label: "Mức giảm",
      search_code_placeholder: "Tìm mã Code...",
      edit_ticket: "✨ Cập Nhật Ticket",
      new_ticket: "🎟 Tạo Ticket Mới",
      cancel_btn: "Hủy Bỏ",
      update_now_btn: "Cập Nhật Ngay",
      publish_ticket_btn: "Phát Hành Ticket",
      coupon_code_label: "🏷 Mã Coupon Đặc Biệt",
      eg_megasale: "VD: MEGASALE",
      discount_percent_label: "✂️ Số % Sẽ Giảm",
      percent_sale: " % SALE",
      start_label: "⏳ Khởi Đầu",
      end_label: "🛑 Kết Thúc",
      unlock_usage: "Mở Khóa Sử Dụng?",
      turn_on_desc: "Bật công tắc để chiến dịch chạy ngay lập tức",
      user_table_role: "Vai trò",
      user_table_date: "Ngày tạo",
      user_table_status: "Trạng thái",
      user_table_action: "Hành động",
      update_user_header: "Chi tiết tài khoản",
      full_name_label: "HỌ VÀ TÊN",
      role_label: "VAI TRÒ",
      admin_role: "Quản trị viên (Admin)",
      customer_role: "Khách hàng (Customer)",
      update_btn: "Cập nhật",
      close_btn: "Đóng",
      confirm_delete_header: "Cảnh báo bảo mật",
      confirm_delete_msg: "Hành động này sẽ xóa vĩnh viễn tài khoản. Bạn có chắc chắn không?",
      bulk_delete_header: "Xác nhận xóa hàng loạt",
      bulk_delete_msg: "Bạn có chắc chắn muốn xóa {{count}} người dùng đã chọn?",
      confirm_yes: "Xác nhận xóa",
      confirm_no: "Quay lại",

      // --- Filter Sidebar ---
      filters: "Bộ lọc",
      gender: "Giới tính",
      men: "Nam",
      price: "Giá",
      clear: "Xóa tất cả",
      apply: "Áp dụng",
      category: "Danh mục sản phẩm",

      // Collections
      group_collections: "Bộ sưu tập đặc biệt",
      special_edition: "Special Edition",
      luxury_collection: "Luxury Collection",
      summer_edition: "Summer Edition",
      unique_collection: "Unique Collection",

      // Colors
      colors_label: "Màu sắc",
      color_black: "Đen (Black)",
      color_red: "Đỏ (Red)",
      color_blue: "Xanh dương (Blue)",
      color_white: "Trắng (White)",
      color_rose: "Hồng (Rose)",
      color_green: "Xanh lá (Green)",

      // Sort
      sort_by: "Sắp xếp theo",
      sort_featured: "Nổi bật",
      sort_newest: "Mới nhất",
      sort_price_high: "Giá: Cao đến Thấp",
      sort_price_low: "Giá: Thấp đến Cao",

      // Ops Dashboard
      new_order_widget: "Đơn hàng mới",
      new_order_desc: "Cần xác nhận ngay",
      shipping_widget: "Đang vận chuyển",
      shipping_desc: "Đơn hàng trên đường đi",
      my_customers_widget: "Khách hàng của tôi",
      total_customers_desc: "Tổng khách hàng",
      completed_widget: "Đã hoàn thành",
      completed_desc: "Đơn hàng thành công",
      ops_dashboard_desc: "Hệ thống vận hành đơn hàng FashionStyle.",

      // --- New Additions (Quyền Hạn, Thống kê, etc) ---
      running_codes_list: "Danh Sách Mã Chạy",
      status_active: "Đang hoạt động",
      switch_account: "Chuyển sang tài khoản",
      staff_profile: "Hồ sơ nhân sự",
      close: "Đóng",
      full_name_upper: "HỌ TÊN",
      system_email_upper: "EMAIL HỆ THỐNG",
      admin_role_upper: "VAI TRÒ QUẢN TRỊ",
      system_settings: "Cài đặt hệ thống",
      default_theme: "Giao diện mặc định",
      theme_notice: "Hệ thống đã được tối ưu hóa cho phong cách trắng sáng. Chế độ tối hiện không khả dụng để đảm bảo trải nghiệm thương hiệu tốt nhất.",
      management: "Quản lý",
      orders_list: "Danh sách Đơn hàng",
      orders_list_desc: "Hệ thống xử lý và vận hành đơn hàng chuyên nghiệp.",
      search_order_placeholder: "Tìm mã ĐH, khách hàng...",
      total_orders_short: "Tổng đơn",
      need_processing: "Cần xử lý",
      shipping_short: "Đang giao",
      status_pending: "Chờ xử lý",
      status_completed: "Đã hoàn thành",
      order_id_col: "MÃ ĐH",
      customer_col: "KHÁCH HÀNG",
      order_date_col: "NGÀY ĐẶT",
      total_amount_col: "TỔNG TIỀN",
      payment_method_col: "P.THỨC",
      status_col: "TRẠNG THÁI",
      order_details: "Chi tiết Đơn hàng",
      customer_info: "Thông tin khách",
      guest: "Khách vãng lai",
      customer_name_label: "Tên khách hàng",
      payment_method_short: "Phương thức",
      ordered_products: "Sản phẩm đã đặt",
      expand_items_desc: "Mở rộng xem chi tiết Items...",
      status_label_colon: "Trạng thái:",
      approve_order_btn: "Duyệt Đơn",
      update_shipping_btn: "Cập nhật vận chuyển",
      approve_order_header: "Phê duyệt Đơn hàng",
      approving_order_msg: "Bạn đang phê duyệt đơn hàng",
      notify_email_msg: "Hành động này sẽ thông báo cho khách qua Email.",
      reject_reason_label: "Lý do từ chối (Chỉ điền nếu Hủy đơn)",
      reject_reason_placeholder: "Khách sẽ nhận email giải thích này...",
      cancel_order_btn: "Hủy Bỏ Đơn",
      approve_now_btn: "Duyệt Ngay",
      update_shipping_header: "Cập nhật Vận chuyển",
      order_label_colon: "Đơn hàng: ",
      status_label_colon_short: "Trạng thái: ",
      select_new_status: "Chọn trạng thái mới",
      new_status_placeholder: "Ví dụ: Đang giao, Đã hoàn thành...",
      save_update_btn: "Lưu & Cập nhật",
      confirm_delete_header2: "Xác nhận xóa",
      confirm_delete_order: "Bạn có chắc chắn muốn xóa đơn hàng",
      delete_order_warning: "Hành động này không thể hoàn tác và sẽ xóa vĩnh viễn dữ liệu đơn hàng.",
      delete_permanently_btn: "Xóa Vĩnh Viễn",
      status_online: "ONLINE",
      level_3: "LEVEL 3",
      permissions_label: "QUYỀN HẠN",
      no_permissions_label: "KHÔNG CÓ QUYỀN",
      ops_activity_log: "Nhật ký Vận hành",
      admin_prefix: "Admin: ",
      no_activity_today: "Chưa có hoạt động nào trong hôm nay",

      // --- Roles Additions ---
      system_active_status: "Hệ thống đang hoạt động",
      log_storage_limit_desc: "Lưu trữ tối đa 100 hành động gần nhất của các Admin",
      search_log_placeholder: "Tìm user, email, hành động...",
      clear_all_logs: "Xóa tất cả nhật ký",
      no_log_found: "Không tìm thấy lịch sử phù hợp.",
      flow_desc: "Sự phối hợp giữa các tài khoản",
      super_admin_panel_title: "Bảng Điều Khiển Độc Quyền (Admin Chính)",
      super_admin_panel_sub: "Trung tâm thao tác các quyền hạn cao cấp nhất của hệ thống",
      approve_pending_orders: "Duyệt Đơn Hàng (Chờ xử lý)",
      no_pending_orders: "Không có đơn hàng chờ duyệt.",
      total_label: "Tổng:",
      approve_btn: "Duyệt",
      reject_btn: "Từ Chối",
      manage_admin_2_3: "Quản Lý Admin 2 & 3",
      admin2_short_name: "Admin Kho (A2)",
      admin2_delete_perm: "Quyền Xóa Sản Phẩm Kho",
      admin3_short_name: "Admin Vận Hành (A3)",
      admin3_access_perm: "Quyền truy cập thông tin khách",
      system_config: "Cấu Hình Hệ Thống",
      maintenance_mode: "Chế độ Bảo Trì Web",
      tgm_alerts: "Nhận cảnh báo qua TGM",
      close_registration: "Đóng đăng ký Customer mới",

      // --- Promotions VIP & Random Codes ---
      assign_vip: "Gán Mã Cho VIP",
      vip_gift_title: "Tặng Quà VIP",
      vip_gift_subtitle: "Tri ân khách hàng thân thiết",
      send_gift_now: "GỬI TẶNG NGAY",
      vip_gift_desc_1: "Chọn 1 khách hàng từ ",
      vip_gift_desc_top: "Top mua hàng",
      vip_gift_desc_2: " và một mã ",
      vip_gift_desc_voucher: "Voucher Đặc quyền",
      vip_gift_desc_3: " để gửi trực tiếp cho họ.",
      gift_recipient: "Đối tượng nhận quà",
      search_vip: "Tìm kiếm khách hàng VIP...",
      orders_count: "đơn",
      voucher_type: "Loại Voucher gán tặng",
      select_promo_code: "Chọn mã khuyến mãi...",
      special_promo: "Khuyến mãi đặc biệt",
      specific_code_opt: "Chọn mã cụ thể (Tùy chọn)",
      select_specific_code: "Chọn một mã Ticket riêng...",
      specific_code_note: "* Nếu bỏ trống, hệ thống sẽ tự động gán chiến dịch chung.",
      random_code_vault: "Kho Mã Random",
      single_use_system: "Hệ thống phát hành mã dùng 1 lần",
      total_codes: "Tổng số mã",
      used_codes: "Đã dùng",
      available_codes: "Khả dụng",
      code_prefix: "Tiền tố mã (Prefix)",
      eg_prefix: "VD: KM-",
      quantity: "Số lượng",
      generate_codes: "TẠO MÃ",
      no_sub_codes: "Chiến dịch này chưa phát hành mã con nào.",
      random_ticket_code: "MÃ TICKET RANDOM",
      code_ready: "Sẵn sàng",
      code_used: "Đã dùng",
      
      msg_confirm_delete_promo: "Bạn có chắc muốn xóa khuyến mãi này?",
      msg_fill_info: "Vui lòng điền đủ thông tin.",
      msg_promo_updated: "Đã cập nhật mã khuyến mãi",
      msg_promo_created: "Đã tạo mã khuyến mãi thành công",
      msg_select_user_promo: "Vui lòng chọn đủ Khách hàng và Chiến dịch",
      msg_fetch_code_error: "Có lỗi xảy ra khi lấy danh sách mã.",
    },
  },
  en: {
    translation: {
      // --- Topbar & Sidebar ---
      search: "Search...",
      search_placeholder: "Search product...",
      home: "Home",
      dashboard: "Dashboard",
      users: "Users",
      roles: "Roles",
      product: "Product",
      products_title: "Products",
      promotions: "Promotions",
      login: "Sign in",

      // --- Dashboard ---
      acc_revenue: "Accumulated Revenue",
      desc_revenue: "Total sales revenue",
      acc_users: "Registered Customers",
      desc_users: "System members",
      acc_orders: "Total Orders",
      desc_orders: "Successful orders",
      acc_reviews: "Total Reviews",
      desc_reviews: "User feedback",
      welcome_back: "Welcome back! 👋",
      dashboard_desc: "Overall system statistics dashboard.",
      trending_categories: "Trending Categories",
      total_products: "TOTAL PROD",
      no_product_data: "No product data available",
      revenue_7_days: "Revenue last 7 days",
      revenue_stats_desc: "Daily revenue statistics",
      no_recent_transactions: "No recent transactions",

      // --- Notifications ---
      notifications: "Notifications",
      notification_empty: "No notifications yet",
      notif_new_order: "New Order",
      notif_new_review: "New Review",
      notif_admin_action: "Admin Activity",
      mark_all_read: "Mark all as read",
      view_all: "View all",

      // --- Profile Popover ---
      profile: "Profile",
      settings: "Settings",
      logout: "Logout",

      // --- Product Page ---
      new_product: "New Product",
      add_product_title: "Add New Product",
      edit_product_title: "Update Product",
      product_name: "Product Name",
      product_price: "Price",
      select_category: "Select Category",
      save: "Save",
      delete: "Delete",
      cancel: "Cancel",
      size_stock_management: "Size Stock Management",
      size_name: "Size (XS, M...)",
      size_stock: "Quantity",
      add_size: "+ Add Size",
      no_sizes: "Empty. Total stock will be 0 if no sizes.",

      // --- Promotions Page ---
      promotions_title: "Promotions",
      promotions_sub: "Manage discount codes and campaigns",
      add_promotion: "Add Promotion",
      promo_code: "Discount Code",
      promo_discount: "Discount Value",
      promo_type: "Type",
      promo_start: "Start Date",
      promo_end: "End Date",
      promo_percent: "Percentage (%)",
      promo_fixed: "Fixed ($)",
      
      promotions_title_big: "Promotion Management",
      promotions_desc_big: "Create and config attractive Voucher campaigns",
      create_campaign: "Create Campaign",
      empty_promotions: "No promotion campaigns created yet.",
      ticket_code: "TICKET CODE",
      discount_percent: "DISCOUNT",
      start_date: "START DATE",
      end_date: "END DATE",
      status: "STATUS",
      actions: "ACTIONS",

      // --- Roles Page ---
      roles_title: "Access Control",
      roles_sub: "Manage 3 Admin levels and activity history",
      activity_log: "Activity Log",
      order_flow: "Order Flow",
      clear_all_logs: "Clear all",

      // --- Roles Constants ---
      role_admin1_title: "Super Admin",
      role_admin1_sub: "Full system control",
      role_admin1_p1: "View & manage entire Dashboard",
      role_admin1_p2: "Manage all users (CRUD)",
      role_admin1_p3: "Manage & approve products",
      role_admin1_p4: "Full control of Promotion campaigns",
      role_admin1_p5: "Approve orders created by Admin 3",
      role_admin1_p6: "Delegate & manage Admin 2, Admin 3",
      role_admin1_p7: "View revenue statistics reports",
      role_admin1_p8: "System configuration",
      
      role_admin2_title: "Inventory Admin",
      role_admin2_sub: "Product & inventory management",
      role_admin2_p1: "Add new products to inventory",
      role_admin2_p2: "Edit product info, price, images",
      role_admin2_p3: "Create Coupon & % Discount campaigns",
      role_admin2_p4: "Delete products (upon Admin 1's request)",
      role_admin2_p5: "View product list & inventory",
      role_admin2_p6: "View inventory statistics",
      role_admin2_b1: "User management",
      role_admin2_b2: "Order approval",
      role_admin2_b3: "Admin delegation",
      role_admin2_b4: "System configuration",
      
      role_admin3_title: "Operations Admin",
      role_admin3_sub: "Order processing & customer care",
      role_admin3_p1: "Receive order info from customers",
      role_admin3_p2: "Create orders pending Admin 1 approval",
      role_admin3_p3: "View customer info for orders",
      role_admin3_p4: "Update shipping status",
      role_admin3_p5: "View order history & list",
      role_admin3_b1: "Product management",
      role_admin3_b2: "Promotion management",
      role_admin3_b3: "Admin delegation",
      role_admin3_b4: "Delete users",
      role_admin3_b5: "System configuration",
      
      flow_step1_actor: "Customer",
      flow_step1_action: "Place order on Web",
      flow_step2_actor: "Ops Admin",
      flow_step2_action: "Create order",
      flow_step3_actor: "Super Admin",
      flow_step3_action: "Approve / Reject order",
      flow_step4_actor: "Ops Admin",
      flow_step4_action: "Deliver via Shipping",
      
      role_admin1_exclusive: "Admin 1 can execute ALL permissions of Admin 2 and Admin 3",
      reset_pw: "Reset PW",
      lock_acc: "Lock Acc",
      unlock_acc: "Unlock Acc",

      // --- User Page ---
      user_management_sub: "System account and permission management",
      user_table_name: "User",
      search_user_placeholder: "Search Name, Phone...",
      users: "Users",
      
      // User Status & Detailed Strings
      status_pending_admin: "Pending Approval",
      status_processing: "Processing",
      status_shipping: "Shipping",
      status_delivered: "Delivered",
      status_cancelled: "Cancelled",
      status_awaiting_payment: "Awaiting Payment",
      status_approved: "Approved",
      delete_account_confirm: "Confirm Account Deletion",
      delete_user_question: "Delete account",
      delete_warning: "This action cannot be undone.",
      accept_label: "Delete",
      reject_label: "Cancel",
      filter_all: "All",
      filter_admin: "Admin",
      filter_customer: "Customer",
      total_users: "Total Users",
      admins_count: "Administrators",
      customers_count: "Customers",
      active_users_count: "Active",
      no_users_found: "No users found",
      role_admin_badge: "Admin",
      role_customer_badge: "Customer",
      created_at: "Created",
      showing_users: "Showing",
      users_count_label: "users",
      personal_info: "Personal Information",
      phone_label: "Phone",
      not_provided: "Not provided",
      address_label: "Address",
      register_date_label: "Register Date",
      status_label: "Status",
      active_status: "Active",
      locked_status: "Locked",
      order_history: "Order History",
      no_orders: "No orders yet",
      order_hash: "Order #",
      products_count_label: "products",
      delete_this_account: "Delete this account",

      // --- Coupons Page ---
      status_running: "Running",
      status_stopped: "Stopped/Expired",
      discount_amount_label: "Discount",
      search_code_placeholder: "Search Code...",
      edit_ticket: "✨ Update Ticket",
      new_ticket: "🎟 New Ticket",
      cancel_btn: "Cancel",
      update_now_btn: "Update Now",
      publish_ticket_btn: "Publish Ticket",
      coupon_code_label: "🏷 Special Coupon Code",
      eg_megasale: "E.g. MEGASALE",
      discount_percent_label: "✂️ Discount Percentage",
      percent_sale: " % SALE",
      start_label: "⏳ Start Date",
      end_label: "🛑 End Date",
      unlock_usage: "Unlock Usage?",
      turn_on_desc: "Toggle to activate the campaign immediately",
      user_table_role: "Role",
      user_table_date: "Created At",
      user_table_status: "Status",
      user_table_action: "Action",
      update_user_header: "Account Details",
      full_name_label: "FULL NAME",
      role_label: "ROLE",
      admin_role: "Administrator (Admin)",
      customer_role: "Customer",
      update_btn: "Update",
      close_btn: "Close",
      confirm_delete_header: "Security Warning",
      confirm_delete_msg: "This action will permanently delete the account. Are you sure?",
      bulk_delete_header: "Confirm Bulk Delete",
      bulk_delete_msg: "Are you sure you want to delete {{count}} selected users?",
      confirm_yes: "Confirm Delete",
      confirm_no: "Go Back",

      // --- Filter Sidebar ---
      filters: "Filters",
      gender: "Gender",
      men: "Men",
      price: "Price",
      clear: "Clear All",
      apply: "Apply",
      category: "Category",

      // Collections
      group_collections: "Special Collections",
      special_edition: "Special Edition",
      luxury_collection: "Luxury Collection",
      summer_edition: "Summer Edition",
      unique_collection: "Unique Collection",

      // Colors
      colors_label: "Colors",
      color_black: "Black",
      color_red: "Red",
      color_blue: "Blue",
      color_white: "White",
      color_rose: "Rose",
      color_green: "Green",

      // Sort
      sort_by: "Sort By",
      sort_featured: "Featured",
      sort_newest: "Newest",
      sort_price_high: "Price: High-Low",
      sort_price_low: "Price: Low-High",

      // Ops Dashboard
      new_order_widget: "New Orders",
      new_order_desc: "Needs immediate confirmation",
      shipping_widget: "Shipping",
      shipping_desc: "Orders on the way",
      my_customers_widget: "My Customers",
      total_customers_desc: "Total customers",
      completed_widget: "Completed",
      completed_desc: "Successful orders",
      ops_dashboard_desc: "FashionStyle Order Operations System.",

      // --- New Additions (Quyền Hạn, Thống kê, etc) ---
      running_codes_list: "Running Codes List",
      status_active: "Active",
      switch_account: "Switch Account",
      staff_profile: "Staff Profile",
      close: "Close",
      full_name_upper: "FULL NAME",
      system_email_upper: "SYSTEM EMAIL",
      admin_role_upper: "ADMIN ROLE",
      system_settings: "System Settings",
      default_theme: "Default Theme",
      theme_notice: "The system is optimized for a light theme. Dark mode is currently unavailable to ensure the best brand experience.",
      management: "Management",
      orders_list: "Orders List",
      orders_list_desc: "Professional order processing and operations system.",
      search_order_placeholder: "Search Order ID, Customer...",
      total_orders_short: "Total Orders",
      need_processing: "Needs Processing",
      shipping_short: "Shipping",
      status_pending: "Pending",
      status_completed: "Completed",
      order_id_col: "ORDER ID",
      customer_col: "CUSTOMER",
      order_date_col: "ORDER DATE",
      total_amount_col: "TOTAL AMOUNT",
      payment_method_col: "PAYMENT",
      status_col: "STATUS",
      order_details: "Order Details",
      customer_info: "Customer Information",
      guest: "Guest",
      customer_name_label: "Customer Name",
      payment_method_short: "Payment Method",
      ordered_products: "Ordered Products",
      expand_items_desc: "Expand to see Item details...",
      status_label_colon: "Status:",
      approve_order_btn: "Approve Order",
      update_shipping_btn: "Update Shipping",
      approve_order_header: "Approve Order",
      approving_order_msg: "You are approving order",
      notify_email_msg: "This action will notify the customer via Email.",
      reject_reason_label: "Rejection Reason (Fill only if Cancelling order)",
      reject_reason_placeholder: "The customer will receive an email explaining this...",
      cancel_order_btn: "Cancel Order",
      approve_now_btn: "Approve Now",
      update_shipping_header: "Update Shipping",
      order_label_colon: "Order: ",
      status_label_colon_short: "Status: ",
      select_new_status: "Select new status",
      new_status_placeholder: "E.g. Shipping, Completed...",
      save_update_btn: "Save & Update",
      confirm_delete_header2: "Confirm Deletion",
      confirm_delete_order: "Are you sure you want to delete order",
      delete_order_warning: "This action cannot be undone and will permanently delete the order data.",
      delete_permanently_btn: "Permanently Delete",
      status_online: "ONLINE",
      level_3: "LEVEL 3",
      permissions_label: "PERMISSIONS",
      no_permissions_label: "NO PERMISSIONS",
      ops_activity_log: "Operations Log",
      admin_prefix: "Admin: ",
      no_activity_today: "No activities today",
      promotions_menu: "Promotions",
      orders_menu: "Orders",
      role_admin1_name: "Super Admin",
      role_admin2_name: "Inventory Admin",
      role_admin3_name: "Operations Admin",
      role_admin1_desc: "Administrator",
      role_admin2_desc: "Inventory Manager",
      role_admin3_desc: "Operations",

      // --- New Additions (Quyền Hạn, Thống kê, etc) ---
      running_codes_list: "Running Codes List",
      status_active: "Active",
      switch_account: "Switch Account",
      staff_profile: "Staff Profile",
      close: "Close",
      full_name_upper: "FULL NAME",
      system_email_upper: "SYSTEM EMAIL",
      admin_role_upper: "ADMIN ROLE",
      system_settings: "System Settings",
      default_theme: "Default Theme",
      theme_notice: "The system is optimized for a light theme. Dark mode is currently unavailable to ensure the best brand experience.",
      management: "Management",
      orders_list: "Orders List",
      orders_list_desc: "Professional order processing and operations system.",
      search_order_placeholder: "Search Order ID, Customer...",
      total_orders_short: "Total Orders",
      need_processing: "Needs Processing",
      shipping_short: "Shipping",
      status_pending: "Pending",
      status_completed: "Completed",
      order_id_col: "ORDER ID",
      customer_col: "CUSTOMER",
      order_date_col: "ORDER DATE",
      total_amount_col: "TOTAL AMOUNT",
      payment_method_col: "PAYMENT",
      status_col: "STATUS",
      order_details: "Order Details",
      customer_info: "Customer Information",
      guest: "Guest",
      customer_name_label: "Customer Name",
      payment_method_short: "Payment Method",
      ordered_products: "Ordered Products",
      expand_items_desc: "Expand to see Item details...",
      status_label_colon: "Status:",
      approve_order_btn: "Approve Order",
      update_shipping_btn: "Update Shipping",
      approve_order_header: "Approve Order",
      approving_order_msg: "You are approving order",
      notify_email_msg: "This action will notify the customer via Email.",
      reject_reason_label: "Rejection Reason (Fill only if Cancelling order)",
      reject_reason_placeholder: "The customer will receive an email explaining this...",
      cancel_order_btn: "Cancel Order",
      approve_now_btn: "Approve Now",
      update_shipping_header: "Update Shipping",
      order_label_colon: "Order: ",
      status_label_colon_short: "Status: ",
      select_new_status: "Select new status",
      new_status_placeholder: "E.g. Shipping, Completed...",
      save_update_btn: "Save & Update",
      confirm_delete_header2: "Confirm Deletion",
      confirm_delete_order: "Are you sure you want to delete order",
      delete_order_warning: "This action cannot be undone and will permanently delete the order data.",
      delete_permanently_btn: "Permanently Delete",
      status_online: "ONLINE",
      level_3: "LEVEL 3",
      permissions_label: "PERMISSIONS",
      no_permissions_label: "NO PERMISSIONS",
      ops_activity_log: "Operations Log",
      admin_prefix: "Admin: ",
      no_activity_today: "No activities today",

      // --- Roles Additions ---
      system_active_status: "System is active",
      log_storage_limit_desc: "Stores up to 100 most recent Admin actions",
      search_log_placeholder: "Search user, email, action...",
      clear_all_logs: "Clear all logs",
      no_log_found: "No matching history found.",
      flow_desc: "Collaboration between accounts",
      super_admin_panel_title: "Exclusive Super Admin Panel",
      super_admin_panel_sub: "Central hub for highest system permissions",
      approve_pending_orders: "Approve Pending Orders",
      no_pending_orders: "No pending orders.",
      total_label: "Total:",
      approve_btn: "Approve",
      reject_btn: "Reject",
      manage_admin_2_3: "Manage Admin 2 & 3",
      admin2_short_name: "Inventory Admin (A2)",
      admin2_delete_perm: "Delete Product Permission",
      admin3_short_name: "Operations Admin (A3)",
      admin3_access_perm: "Access Customer Info Permission",
      system_config: "System Configuration",
      maintenance_mode: "Web Maintenance Mode",
      tgm_alerts: "Receive TGM Alerts",
      close_registration: "Close Customer Registration",

      // --- Promotions VIP & Random Codes ---
      assign_vip: "Assign VIP Code",
      vip_gift_title: "VIP Gift",
      vip_gift_subtitle: "Appreciate loyal customers",
      send_gift_now: "SEND GIFT NOW",
      vip_gift_desc_1: "Choose 1 customer from ",
      vip_gift_desc_top: "Top Buyers",
      vip_gift_desc_2: " and an ",
      vip_gift_desc_voucher: "Exclusive Voucher",
      vip_gift_desc_3: " to send directly to them.",
      gift_recipient: "Gift Recipient",
      search_vip: "Search VIP customer...",
      orders_count: "orders",
      voucher_type: "Type of Voucher to assign",
      select_promo_code: "Select promotion code...",
      special_promo: "Special promotion",
      specific_code_opt: "Select specific code (Optional)",
      select_specific_code: "Select a unique Ticket code...",
      specific_code_note: "* If left blank, the system will automatically assign the general campaign.",
      random_code_vault: "Random Code Vault",
      single_use_system: "Single-use code issuance system",
      total_codes: "Total codes",
      used_codes: "Used",
      available_codes: "Available",
      code_prefix: "Code prefix (Prefix)",
      eg_prefix: "E.g. KM-",
      quantity: "Quantity",
      generate_codes: "GENERATE CODES",
      no_sub_codes: "This campaign has not issued any sub-codes yet.",
      random_ticket_code: "RANDOM TICKET CODE",
      code_ready: "Ready",
      code_used: "Used",
      
      msg_confirm_delete_promo: "Are you sure you want to delete this promotion?",
      msg_fill_info: "Please fill in all information.",
      msg_promo_updated: "Promotion updated successfully",
      msg_promo_created: "Promotion created successfully",
      msg_select_user_promo: "Please select both Customer and Campaign",
      msg_fetch_code_error: "An error occurred while fetching the code list.",
    },
  },
};

i18n.use(initReactI18next).init({
  resources,
  lng: "vi",
  fallbackLng: "en",
  interpolation: { escapeValue: false },
});

export default i18n;

...
| Bảng 3.1 | Các nhóm bảng cơ sở dữ liệu | 29 |
| Bảng 4.1 | Yêu cầu phần mềm cài đặt | 30 |
| Bảng 4.2 | Kiểm thử — Module xác thực | 44 |
| Bảng 4.3 | Kiểm thử — Module đơn hàng | 46 |
| Bảng 4.4 | Kiểm thử — Ứng dụng di động | 48 |
...
| Từ viết tắt | Nghĩa đầy đủ tiếng Anh | Giải nghĩa tiếng Việt |
|------------|------------------------|----------------------|
| ADB | Android Debug Bridge | Cầu nối gỡ lỗi Android — công cụ kết nối máy tính với thiết bị/máy ảo Android |
| ACID | Atomicity, Consistency, Isolation, Durability | Tính Nguyên tử, Nhất quán, Cô lập, Bền vững — 4 tính chất của giao dịch CSDL |
| AGP | Android Gradle Plugin | Plugin Gradle dành cho Android — công cụ quản lý quá trình build APK |
| API | Application Programming Interface | Giao diện lập trình ứng dụng — tập hợp quy tắc để các phần mềm giao tiếp |
| APK | Android Package Kit | Gói cài đặt ứng dụng Android |
| AVD | Android Virtual Device | Thiết bị Android ảo — máy ảo mô phỏng điện thoại chạy trong Android Studio |
| AWS | Amazon Web Services | Dịch vụ đám mây của Amazon (dùng để so sánh, ngoài phạm vi đồ án) |
| COD | Cash on Delivery | Thanh toán khi nhận hàng — hình thức thanh toán được hỗ trợ trong FashionStyle |
| CPU | Central Processing Unit | Đơn vị xử lý trung tâm (bộ vi xử lý) |
| CRUD | Create, Read, Update, Delete | Tạo, Đọc, Cập nhật, Xóa — 4 thao tác cơ bản với cơ sở dữ liệu |
| CSS | Cascading Style Sheets | Ngôn ngữ định kiểu giao diện web (Tailwind CSS được dùng trong dự án) |
| DB | Database | Cơ sở dữ liệu |
| DFD | Data Flow Diagram | Sơ đồ Luồng Dữ Liệu — biểu đồ thể hiện luồng xử lý thông tin trong hệ thống |
| DOM | Document Object Model | Mô hình Đối tượng Tài liệu — cấu trúc cây của trang HTML, React dùng Virtual DOM |
| ENV | Environment | Môi trường / tệp biến môi trường (`.env`) chứa cấu hình bảo mật của backend |
| ERD | Entity Relationship Diagram | Sơ đồ Thực Thể Liên Kết — mô hình hóa cấu trúc cơ sở dữ liệu |
| FCM | Firebase Cloud Messaging | Dịch vụ thông báo đẩy của Google (ngoài phạm vi đồ án này) |
| FR | Functional Requirement | Yêu cầu Chức năng |
| GIT | Global Information Tracker | Hệ thống quản lý phiên bản mã nguồn phân tán |
| HMR | Hot Module Replacement | Thay thế module nóng — tính năng cập nhật tức thì khi lập trình với Vite |
| HTML | Hypertext Markup Language | Ngôn ngữ đánh dấu siêu văn bản — ngôn ngữ cấu trúc trang web |
| HTTP | Hypertext Transfer Protocol | Giao thức Truyền Siêu Văn Bản — giao thức giao tiếp giữa client và server |
| HTTPS | Hypertext Transfer Protocol Secure | Giao thức HTTP có mã hóa bảo mật TLS/SSL |
| I/O | Input / Output | Vào / Ra — các thao tác nhập xuất dữ liệu (Node.js xử lý I/O bất đồng bộ) |
| IDE | Integrated Development Environment | Môi trường Phát triển Tích hợp — phần mềm lập trình (VS Code, Android Studio) |
| i18n | Internationalization | Quốc tế hóa — hỗ trợ đa ngôn ngữ Anh/Việt thông qua thư viện i18next |
| IT | Information Technology | Công nghệ Thông tin |
| JSON | JavaScript Object Notation | Định dạng trao đổi dữ liệu dạng văn bản, dùng trong toàn bộ REST API |
| JWT | JSON Web Token | Mã thông báo xác thực phi trạng thái sử dụng chữ ký số |
| MVC | Model-View-Controller | Kiến trúc phần mềm phân tách dữ liệu, giao diện và điều khiển |
| NFR | Non-Functional Requirement | Yêu cầu Phi chức năng (hiệu năng, bảo mật, khả năng mở rộng) |
| npm | Node Package Manager | Trình quản lý gói phần mềm của Node.js |
| OTP | One-Time Password | Mật khẩu dùng một lần — dùng trong tính năng xác thực quên mật khẩu |
| OUM | Open University Malaysia | Đại học Mở Malaysia |
| RBAC | Role-Based Access Control | Kiểm Soát Truy Cập Dựa Trên Vai Trò — phân quyền 3 cấp Admin trong hệ thống |
| RDBMS | Relational Database Management System | Hệ Quản trị Cơ sở Dữ liệu Quan hệ (MySQL 8.0 được sử dụng trong dự án) |
| RAD | Rapid Application Development | Phát triển Ứng dụng Nhanh — phương pháp phát triển lặp được áp dụng trong đồ án |
| REST | Representational State Transfer | Kiến trúc Truyền Trạng Thái Đại Diện — tiêu chuẩn thiết kế API web |
| SDK | Software Development Kit | Bộ Công cụ Phát triển Phần mềm (Android SDK dùng để build APK) |
| SDLC | Software Development Life Cycle | Vòng đời Phát triển Phần mềm |
| SME | Small and Medium-sized Enterprise | Doanh nghiệp Vừa và Nhỏ — đối tượng hưởng lợi mà hệ thống nhắm tới |
| SMTP | Simple Mail Transfer Protocol | Giao thức Truyền Thư Đơn giản — dùng để gửi email qua Gmail SMTP (Nodemailer) |
| SQL | Structured Query Language | Ngôn ngữ Truy vấn Có Cấu trúc — dùng để truy vấn cơ sở dữ liệu MySQL |
| TCP | Transmission Control Protocol | Giao thức Kiểm soát Truyền tin — nền tảng kết nối mạng đáng tin cậy |
| UC | Use Case | Trường hợp Sử dụng — mô tả tương tác giữa tác nhân và hệ thống |
| UI | User Interface | Giao diện Người dùng |
| URL | Uniform Resource Locator | Định vị Tài nguyên Thống nhất — địa chỉ web |
| USD | United States Dollar | Đô la Mỹ — đơn vị tiền tệ dùng trong báo cáo chi phí |
| UX | User Experience | Trải nghiệm Người dùng |
| VIP | Very Important Person | Khách hàng đặc biệt — nhận mã giảm giá cá nhân hóa qua bảng UserVouchers |
| VS Code | Visual Studio Code | Phần mềm soạn thảo mã nguồn mã nguồn mở của Microsoft |
...
**Bảng 2.4 — Kế hoạch thực hiện (7 giai đoạn)**
...
**Bảng 2.3 — Phân tích chi phí – lợi ích**
...
**Bảng 2.1 — Yêu cầu chức năng (YC01–YC10)**
...
**Bảng 2.2 — Yêu cầu phi chức năng (YCP01–YCP05)**
...
**Bảng 2.5 — Lược đồ cơ sở dữ liệu: Bảng Users**

| Cột | Kiểu dữ liệu | Ràng buộc |
|-----|-------------|-----------|
| UserID | INT | KHÓA CHÍNH, TỰ TĂNG |
| FullName | VARCHAR(100) | KHÔNG NULL |
| Email | VARCHAR(100) | DUY NHẤT, KHÔNG NULL |
| PasswordHash | VARCHAR(255) | KHÔNG NULL |
| Address | VARCHAR(255) | NULL |
| PhoneNumber | VARCHAR(15) | NULL |
| Role | VARCHAR(20) | MẶC ĐỊNH 'Customer' |
| IsActive | TINYINT(1) | MẶC ĐỊNH 1 |
| CanDeleteProduct | TINYINT(1) | MẶC ĐỊNH 0 |
| CanAccessCustomerInfo | TINYINT(1) | MẶC ĐỊNH 1 |
| CreatedAt | DATETIME | MẶC ĐỊNH THỜI GIAN HIỆN TẠI |

**Bảng 2.6 — Lược đồ cơ sở dữ liệu: Bảng Products**

| Cột | Kiểu dữ liệu | Ràng buộc |
|-----|-------------|-----------|
| ProductID | INT | KHÓA CHÍNH, TỰ TĂNG |
| ProductName | VARCHAR(255) | KHÔNG NULL |
| Description | TEXT | NULL |
| Price | DECIMAL(10,2) | KHÔNG NULL |
| DiscountPercent | INT | MẶC ĐỊNH 0 |
| StockQuantity | INT | KHÔNG NULL, MẶC ĐỊNH 0 |
| CategoryID | INT | KHÓA NGOẠI → Categories |
| CreatedAt | DATETIME | MẶC ĐỊNH THỜI GIAN HIỆN TẠI |

**Bảng 2.7 — Lược đồ cơ sở dữ liệu: Bảng Orders**
...
| OrderDate | DATETIME | MẶC ĐỊNH THỜI GIAN HIỆN TẠI |
| TotalAmount | DECIMAL(12,2) | KHÔNG NULL |
| Status | VARCHAR(50) | KHÔNG NULL, MẶC ĐỊNH 'Chờ xử lý' |
| ShippingAddress | VARCHAR(255) | KHÔNG NULL |
...
| Cột | Kiểu dữ liệu | Ràng buộc |
|-----|-------------|-----------|
| PromotionID | INT | KHÓA CHÍNH, TỰ TĂNG |
| Code | VARCHAR(50) | DUY NHẤT, KHÔNG NULL |
| DiscountPercent | DECIMAL(5,2) | KHÔNG NULL |
| StartDate | DATETIME | KHÔNG NULL |
| EndDate | DATETIME | KHÔNG NULL |
| IsActive | BOOLEAN | KHÔNG NULL, MẶC ĐỊNH 1 |

**Bảng 2.8 — Lược đồ cơ sở dữ liệu: Bảng UserVouchers**

| Cột | Kiểu dữ liệu | Ràng buộc |
|-----|-------------|-----------|
| VoucherID | INT | KHÓA CHÍNH, TỰ TĂNG |
| UserID | INT | KHÔNG NULL, KHÓA NGOẠI → Users |
| PromotionID | INT | KHÔNG NULL, KHÓA NGOẠI → Promotions |
...
**Bảng 3.1 — Các nhóm bảng cơ sở dữ liệu**
...
**Bảng 4.1 — Yêu cầu phần mềm cài đặt**
...
**Bảng 4.2 — Kiểm thử: Module xác thực**
...
**Bảng 4.3 — Kiểm thử: Module đơn hàng**
...
**Bảng 4.4 — Kiểm thử: Ứng dụng di động**
